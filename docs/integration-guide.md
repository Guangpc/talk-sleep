...
8. 仅当音频为 10 秒至 5 分钟且不超过 20 MiB 时才通过本地校验。上传前必须逐项确认声音授权、限定用途、gateway/MiniMax 云端处理，以及数据保留与删除规则。
...
```json
{
  ...
  "consent": {
    ...
    "acceptedAt": "2026-09-05T03:30:00Z"
  }
}
```

gateway 只接受 `voice_clone` / `prompt_audio`，校验安全文件名（允许中间空格）、扩展名、base64、大小和已验证时长；`voice_clone` 还必须携带完整且可解析时间的 consent attestation，否则不会调用 provider。成功只返回 `{ "fileId": "..." }`。
...
当前已完成 App 逐项确认和 gateway consent attestation，但没有完成授权证明长期归档、provider-side 删除传播、好友 profile/确认记忆持久化、离线重试策略、句子级流式 TTS、锁屏/后台音频和本轮 picker/静默结束修复的真机回归。真机验证步骤留给设备可用时执行；自动测试结果见 [`test-matrix.md`](test-matrix.md)。
