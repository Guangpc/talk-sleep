...
- **本地语音回复：PASS**：`AVSpeechSynthesizer` 自动朗读本地测试回复。
...
| 场景 | 通过标准 | 结果 |
|---|---|---|
| 新款 iPhone · 前台会话 | 自动监听→ASR→AI 朗读 正常 | 部分 PASS：真实麦克风→Apple Speech→本地测试文案→系统 TTS；生产 AI 未接入 |
...
| AI 播放中用户打断（开头/中段/结尾） | 立即降音停止 TTS，保留已显示文字，优先处理新发言 | 部分 PASS：普通音量 3/3；开头/中段/结尾分层仍待测 |
